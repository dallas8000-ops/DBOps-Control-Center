"""Railway hosting cost vs MRR monitor — alerts when revenue exceeds profit threshold."""
...
RAILWAY_TIERS: list[dict[str, Any]] = [
    {"name": "Hobby", "cost": 5, "max_clients": 2, "upgrade_at_revenue": 15},
    {"name": "Pro", "cost": 20, "max_clients": 8, "upgrade_at_revenue": 60},
    {"name": "Pro Plus", "cost": 50, "max_clients": 20, "upgrade_at_revenue": 150},
    {"name": "Team", "cost": 100, "max_clients": 50, "upgrade_at_revenue": 300},
    {"name": "Scale", "cost": 200, "max_clients": 100, "upgrade_at_revenue": 600},
]
...
def _env_float_with_legacy(primary: str, legacy: str, default: float) -> float:
...
def hosting_current_cost() -> float:
    return _env_float_with_legacy("RAILWAY_CURRENT_COST", "RENDER_CURRENT_COST", 20.0)
...
def get_current_hosting_tier() -> dict[str, Any]:
    cost = hosting_current_cost()
    for tier in RAILWAY_TIERS:
        if tier["cost"] == cost:
            return tier
    closest = min(RAILWAY_TIERS, key=lambda t: abs(t["cost"] - cost))
    return closest


def get_next_hosting_tier() -> dict[str, Any] | None:
    cost = hosting_current_cost()
    for index, tier in enumerate(RAILWAY_TIERS):
        if tier["cost"] == cost and index + 1 < len(RAILWAY_TIERS):
            return RAILWAY_TIERS[index + 1]
    return None
...
def _get_recommendation(mrr: float, threshold: float, next_tier: dict[str, Any] | None, net: float) -> str:
    ...
    if net < 0:
        return f"Still pre-profit. Need ${abs(net):.0f}/mo more revenue to break even after Railway and Stripe fees."
    if mrr < threshold:
        remaining = threshold - mrr
        return (
            f"Profitable! Need ${remaining:.0f}/mo more revenue before upgrading Railway "
            f"(at {profit_multiplier():.0f}x rule)."
        )
    if next_tier:
        return (
            f"UPGRADE NOW — Revenue (${mrr:.0f}/mo) is {mrr / threshold:.1f}x your Railway cost. "
            f"Upgrade to Railway {next_tier['name']} (${next_tier['cost']}/mo) for better reliability."
        )
    return "You're on the max Railway tier. Consider moving to dedicated infrastructure."


def check_upgrade_needed(mrr: float, client_count: int) -> dict[str, Any]:
    current_tier = get_current_hosting_tier()
    next_tier = get_next_hosting_tier()
    hosting_cost = hosting_current_cost()
    upgrade_threshold = hosting_cost * profit_multiplier()
    stripe_fees = mrr * STRIPE_FEE_RATE
    net_profit = mrr - hosting_cost
    ...
    clients_to_breakeven = max(0, int(-(-int(hosting_cost) // int(starter_price))))
...
def _alert_event_key(hosting_cost: float) -> str:
    return f"railway_upgrade_alert_{int(hosting_cost)}"


def alert_already_sent(db: Session, hosting_cost: float) -> bool:
    return (
        db.query(OnboardingEvent)
        .filter(OnboardingEvent.event_key == _alert_event_key(hosting_cost))
        .first()
        is not None
    )
...
def record_alert_sent(db: Session, *, actor_user_id: int | None, status: dict[str, Any]) -> None:
    db.add(
        OnboardingEvent(
            event_key=_alert_event_key(hosting_current_cost()),
            actor_user_id=actor_user_id,
            details_json=str(
                {
                    "mrr": status["mrr"],
                    "next_tier": status["next_tier"],
                    "hosting_cost": status["hosting_cost"],
                }
            ),
        )
    )
...
def send_upgrade_alert(status: dict[str, Any]) -> bool:
    if not smtp_configured():
        logger.info("hosting_monitor: SMTP not configured — skipping email alert")
        return False

    subject = f"Time to upgrade Railway — MRR ${status['mrr']}/mo"
    body = f"""Hi Barney,
...
Railway cost:         ${status['hosting_cost']}
...
Current Railway tier: {status['current_tier']} (${status['hosting_cost']}/mo)
Upgrade to:           {status['next_tier']} (${status['next_tier_cost']}/mo)

Action: railway.app → Project → Settings → Usage / Plan

— DBOps Railway Monitor
{status['timestamp']} UTC
""".strip()

    try:
        send_smtp_text_email(to_addr=alert_email(), subject=subject, body=body)
        logger.info("hosting_monitor: alert sent to %s", alert_email())
        return True
    except Exception as exc:
        logger.exception("hosting_monitor: email failed: %s", exc)
        return False


def evaluate_hosting_monitor(db: Session, *, actor_user_id: int | None = None, send_alert: bool = False) -> dict[str, Any]:
    ...
    if status["should_upgrade"] and send_alert:
        if alert_already_sent(db, hosting_current_cost()):
            status["alert_skipped_reason"] = "Alert already sent for this Railway cost tier"
        else:
...
def scheduled_hosting_monitor_check(db: Session) -> dict[str, Any] | None:
    disable = os.getenv("RAILWAY_MONITOR_DISABLE", "").strip() or os.getenv("RENDER_MONITOR_DISABLE", "").strip()
    if disable.lower() in {"1", "true", "yes"}:
        return None
    return evaluate_hosting_monitor(db, actor_user_id=None, send_alert=True)
